...
wson_parser::wson_parser(const char *data) {
    ...
    this->wsonBuffer = wson_io_buffer_from((void *) data, 1024*1024);
}

wson_parser::wson_parser(const char *data, int length) {
    this->wsonBuffer = wson_io_buffer_from((void *) data, length);
}
...
std::string wson_parser::nextMapKeyUTF8(){
    int keyLength = wson_io_read_var_uint(wsonBuffer);
    uint16_t * utf16 = ( uint16_t *)wson_io_read_bts(wsonBuffer, keyLength);
    std::string str;
...
void wson_parser::toJSONtring(std::string &builder){
    uint8_t  type = wson_io_read_type(wsonBuffer);
    switch (type) {
        case WSON_UINT8_STRING_TYPE: {
            int size = wson_io_read_var_uint(wsonBuffer);
            uint8_t *utf8 = wson_io_read_bts(wsonBuffer, size);
        ...
        case WSON_NUMBER_BIG_DECIMAL_TYPE: {
                int size = wson_io_read_var_uint(wsonBuffer);
                uint16_t *utf16 = (uint16_t *) wson_io_read_bts(wsonBuffer, size);
        ...
        case WSON_NUMBER_INT_TYPE: {
                int32_t num = wson_io_read_var_int(wsonBuffer);
                wson::str_append_number(builder, num);
            }
            return;
        case WSON_NUMBER_FLOAT_TYPE: {
                float num = wson_io_read_float(wsonBuffer);
                wson::str_append_number(builder, num);
            }
            return;
        case WSON_NUMBER_DOUBLE_TYPE: {
               double num = wson_io_read_double(wsonBuffer);
               wson::str_append_number(builder, num);
            }
            return;
        case WSON_NUMBER_LONG_TYPE: {
               int64_t num = wson_io_read_long(wsonBuffer);
               wson::str_append_number(builder, num);
            }
            return;
        ...
        case WSON_BOOLEAN_TYPE_FALSE:
            ...
            return;
        case WSON_MAP_TYPE:{
                    int length = wson_io_read_var_uint(wsonBuffer);
                    builder.append("{");
...
double wson_parser::nextNumber(uint8_t type) {
    switch (type) {
        case WSON_UINT8_STRING_TYPE: {
            int size = wson_io_read_var_uint(wsonBuffer);
            std::string str;
            wson_io_read_bts(wsonBuffer, size);
            uint8_t *utf8 = wson_io_read_bts(wsonBuffer, size);
            str.append(reinterpret_cast<char *>(utf8), size);
...
void wson_parser::skipValue(uint8_t type) {
    switch (type) {
        ...
        case WSON_NULL_TYPE:
            break;
        case WSON_NUMBER_INT_TYPE:
             wson_io_read_var_int(wsonBuffer);
            return;
        case WSON_NUMBER_FLOAT_TYPE:
            (wsonBuffer);
            return;
        case WSON_NUMBER_DOUBLE_TYPE:
            wson_io_read_double(wsonBuffer);
            return;
        case WSON_NUMBER_LONG_TYPE:
             wson_io_read_long(wsonBuffer);
            return;
        case WSON_BOOLEAN_TYPE_TRUE:
            return;
        case WSON_BOOLEAN_TYPE_FALSE:
        ...
        case WSON_MAP_TYPE:{
                ...
                for(int i=0; i<length; i++){
                    int keyLength = wson_io_read_var_uint(wsonBuffer);
                    wson_io_read_bts(wsonBuffer, keyLength);
                    skipValue(wson_io_read_type(wsonBuffer));
                }
            }
            return;
        case WSON_ARRAY_TYPE:{
