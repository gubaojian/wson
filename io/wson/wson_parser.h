...
public:
     ...
     * */
    inline bool hasNext(){
        return wson_io_has_next(this->wsonBuffer);
    }
    ...
    inline uint8_t nextType(){
        if(wsonBuffer->data && wson_io_has_next(wsonBuffer)){
            return  wson_io_read_type(wsonBuffer);
        }
    ...
    inline  int  nextMapSize(){
        return wson_io_read_var_uint(wsonBuffer);
    }
