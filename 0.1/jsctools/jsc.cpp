...
static EncodedJSValue JSC_HOST_CALL functionParseTson(ExecState* exec);
static EncodedJSValue JSC_HOST_CALL functionTsonInit(ExecState* exec);
static EncodedJSValue JSC_HOST_CALL functionTsonDestroy(ExecState* exec);

static EncodedJSValue JSC_HOST_CALL functionTson(ExecState* exec);

static EncodedJSValue JSC_HOST_CALL functionMultiThreadTest(ExecState* exec);

static GlobalObject* globalObjects;
static VM*  globalVM;
...
EncodedJSValue JSC_HOST_CALL functionTsonDestroy(ExecState* exec){
    printf("destory %p", exec);
    tson::destory();
    return JSValue::encode(jsBoolean(true));
}
...
EncodedJSValue JSC_HOST_CALL functionTson(ExecState* exec)
{
    ...
    auto scope = DECLARE_THROW_SCOPE(vm);
    JSValue value = exec->argument(0);
    tson_buffer* buffer = NULL;
    double start = now_ms();
    for(int i=0; i<10000; i++){
        if(buffer){
           free(buffer);
        }
        buffer = toTson(exec, value);
    }
    printf("tson to tson used %f ms  data length %d \n", (now_ms()-start), buffer->position);
    ...
    start = now_ms();
    for(int i=0; i<10000; i++){
         string = JSONStringify(exec, value, 0);
    }
    printf("json to string used %f ms \n", (now_ms()-start));
    printf("json byte length  %zu \n", string.utf8().length());
...
void* threadRun(void* ptr){
    ...
    const char* code = "a += 1";
    for(int i=0; i<100; i++){
        JSLockHolder lock(globalVM);
        printf("threadRun get lock\n");
        GlobalObject* globalObject = globalObjects;
        ExecState* exec =globalObject->globalExec();
        String sourceCode = String::fromUTF8(code, strlen(code));
        NakedPtr<Exception> evaluationException;
        JSValue result = evaluate(globalObject->globalExec(), makeSource(sourceCode, exec->callerSourceOrigin()), JSValue(), evaluationException);
        if (evaluationException){
            printf("evalute error");
            continue;
        }
        printf("thread %lu %p\n", (unsigned long)pthread_self(), ptr);
        if(result.isString()){
            JSString* jsString = asString(result);
            String resultStr = jsString->value(exec);
            printf("thread %s  \n", resultStr.utf8().data());
     ...
     printf("threadRun end\n");
    return NULL;
}

EncodedJSValue JSC_HOST_CALL functionMultiThreadTest(ExecState* exec)
{
    
    JSLockHolder globalLock(globalVM);
    for(int i=0; i<6; i++){
        pthread_t ntid;
        pthread_attr_t attr;
        pthread_attr_init (&attr);
        int error = pthread_create(&ntid, &attr, &threadRun, NULL);
        if(error != 0){
            printf("create pthread error, %d", error);
        }
